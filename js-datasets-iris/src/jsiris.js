...
(function(jss){
    ...
    for(var i=0; i < jss.rowCount; ++i){
        var row = jss.data[i];
        jss.sepalLength.push(row[0]);
        jss.sepalWidth.push(row[1]);
        jss.petalLength.push(row[2]);
        jss.petalWidth.push(row[3]);
        jss.species.push(row[4]);
        
        var found = false;
        for(var j=0; j < jss.speciesClasses.length; j++){
            if(jss.speciesClasses[j] == row[4]){
                found = true;
                break;
            }
        }
